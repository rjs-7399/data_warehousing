...
df = pd.DataFrame(
    sf_df,
    columns=['YEAR', 'TOTAL_REVENUE', 'TOTAL_ORDERS', 'AVG_REVENUE_PER_ORDER', 'AVG_REVENUE_PER_ITEM',
             'MAX_ORDER_VALUE']
)
...
with col2:
    st.metric("Avg Revenue per Order", f"₹{avg_revenue_per_order:,.0f}",
              delta=f"₹{avg_rev_order_diff:,.0f}" if avg_rev_order_diff is not None else None)
    st.metric("Avg Revenue per Item", f"₹{avg_revenue_per_item:,.0f}",
              delta=f"₹{avg_rev_item_diff:,.0f}" if avg_rev_item_diff is not None else None)

with col3:
    st.metric("Max Order Value", f"₹{max_order_value:,.0f}",
              delta=f"₹{max_order_diff:,.0f}" if max_order_diff is not None else None)
...
trend_chart = alt.Chart(month_df).mark_line(color="#ff5200", point=alt.OverlayMarkDef(color="#ff5200")).encode(
    x=alt.X('Month', sort='ascending', title='Month'),
    y=alt.Y('Total Monthly Revenue', title='Revenue (₹)',
            scale=alt.Scale(domain=[0, month_df['Total Monthly Revenue'].max()])),
    tooltip=[
