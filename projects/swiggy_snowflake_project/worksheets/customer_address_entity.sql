/*------------------------------------------------------------------------------
    CUSTOMER ADDRESS DATA WAREHOUSE ETL WORKFLOW

    This script implements a complete ETL process for customer address data in Snowflake,
...
/*------------------------------------------------------------------------------
                                    System Steps
------------------------------------------------------------------------------*/
-- Create a table for customer address entity in stage layer
-- Create a stream object for customer address entity in stage layer
-- Create a table for customer address entity in clean layer
-- Stage Layer
    -- create table
...
-- Create Customer Address table in stage schema with text data types
...
-- Create append-only stream on customer address table for change data capture
...
-- Create Customer Address table in clean schema with proper data types
-- This represents the single source of truth with validation and standardization
create or replace table clean_sch.customeraddress (
...
-- Create stream on clean customer address table to track changes
...
-- Create Customer Address dimension table in consumption schema
-- Implements SCD Type 2 pattern for historical tracking
create or replace table consumption_sch.customeraddress_dim (
...
comment = 'Customer Address Dimension table with SCD Type 2 handling for historical tracking.';
...
MERGE INTO
    CLEAN_SCH.CUSTOMERADDRESS AS clean
USING
(
    SELECT
...
MERGE INTO
    CONSUMPTION_SCH.CUSTOMERADDRESS_DIM AS target
USING
    CLEAN_SCH.CUSTOMERADDRESS_STM AS source
ON
...
list @csv_stg/delta/customer-address;

-- Load delta customer address data
...
from (
    ...
    from @stage_sch.csv_stg/delta/customer-address/day-02-customer-address-book.csv t
)
...
-- DROP STREAM stage_sch.customeraddress_stm;
...
-- select * from clean_sch.customeraddress;
-- select * from clean_sch.customeraddress_stm;

-- Consumption layer validation queries
-- select * from consumption_sch.customeraddress_dim;