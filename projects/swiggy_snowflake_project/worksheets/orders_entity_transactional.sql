/*------------------------------------------------------------------------------
    ORDERS DATA WAREHOUSE ETL WORKFLOW

    This script implements a complete ETL process for order data in Snowflake,
...
/*------------------------------------------------------------------------------
                                    System Steps
------------------------------------------------------------------------------*/
-- Create a table for orders entity in stage layer
-- Create a stream object for orders entity in stage layer
-- Create a table for orders entity in clean layer
-- Stage Layer
    -- create table
...
-- Create Orders table in stage schema with text data types
...
-- Create append-only stream on orders table for change data capture
...
-- Create Orders table in clean schema with proper data types
...
-- Create standard stream on clean orders table
-- Tracks all DML operations (insert, update, delete) for processing
...
merge into
    clean_sch.orders as target
using
    stage_sch.orders_stm as source
on
    target.ORDER_ID = TRY_TO_NUMBER(source.ORDERID)
when matched then
    update set
...
-- DROP TABLE stage_sch.orders;
-- DROP STREAM stage_sch.orders_stm;
-- DROP TABLE clean_sch.orders;
-- DROP STREAM clean_sch.orders_stm;
