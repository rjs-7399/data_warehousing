/*------------------------------------------------------------------------------
    DELIVERY DATA WAREHOUSE ETL WORKFLOW

    This script implements a complete ETL process for delivery data in Snowflake,
...
/*------------------------------------------------------------------------------
                                    System Steps
------------------------------------------------------------------------------*/
-- Create a table for delivery entity in stage layer
-- Create a stream object for delivery entity in stage layer
-- Create a table for delivery entity in clean layer
-- Stage Layer
    -- create table
...
-- Load Data
    -- List the files in source location
...
-- Create Delivery table in stage schema with text data types
...
-- Create append-only stream on delivery table for change data capture
...
-- Create Delivery table in clean schema with proper data types
...
-- Create stream on clean delivery table
-- Tracks all DML operations for downstream processing
...
/*------------------------------------------------------------------------------
    PART 4: DATA LOADING PROCESS
------------------------------------------------------------------------------*/

-- List files in delta directory to check for available data
list @stage_sch.csv_stg/delta/delivery/;

-- Load delivery data from external files into stage layer
...
from  (
    ...
    from @stage_sch.csv_stg/delta/delivery/day-02-delivery.csv t
)
...
-- DROP TABLE stage_sch.delivery;
-- DROP STREAM stage_sch.delivery_stm;
-- DROP TABLE clean_sch.delivery;
